...
export default function BetriebDashboard() {
  ...
  const ic = isVintage ? "#C49A2A" : "#fbbf24";          // icon / accent color
  const tx = isVintage ? "#E8D070" : "#f4f4f5";           // primary text
  const tm = isVintage ? "#7A5C12" : "#71717a";           // muted text
  const tb = isVintage ? "#C8A86A" : "#d4d4d8";           // body text
  const card = isVintage ? { background: "#130A04", border: "1px solid #7A5C1244" } : { background: "#18181b", border: "1px solid #27272a" };
  const sub  = isVintage ? { background: "#1C0E06", border: "1px solid #7A5C1222" } : { background: "#27272a" };
  const inp  = isVintage ? { background: "#1C0E0688", border: "1px solid #7A5C1233", color: "#C8A86A" } : { background: "#27272a", border: "1px solid #3f3f46", color: "#d4d4d8" };
  const div  = isVintage ? "#7A5C1222" : "#27272a";
  ...
  const wrapperClass = `min-h-screen max-w-sm mx-auto px-5 pb-16 ${isVintage ? "relative z-[2]" : ""}`;
                        ...
                        <div className="mx-4 mb-3 rounded-xl p-4 space-y-3" style={sub}>
                          ...
                          <div className="grid grid-cols-3 gap-2 text-center">
                            {[{ label: "Aktuell", value: membership.currentStamps }, { label: "Gesamt", value: membership.totalStampsEver }, { label: "Belohnt", value: membership.rewardsRedeemed }].map(({ label, value }) => (
                              <div key={label} className="rounded-xl py-2.5" style={isVintage ? { background: "#13070266" } : { background: "#3f3f46" }}>
